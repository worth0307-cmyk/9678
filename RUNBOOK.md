...
**当前分析用不到**——`scripts/20_frequency.py` 证明了日线优于 4h 优于 1h，
而且在零成本下依然如此（1.17 / 1.02 / 0.94），所以不是手续费问题。

但如果你想**先存起来备用**（以后有新想法不用再去抓），加 `--gzip`：

```bash
python -m vibt.ingest "<导出目录>" --gzip
```

压缩比约 **3.7x**（1h 单个币从 1.8MB 降到 0.49MB），26 个币三周期从约 50MB 降到约 14MB。
`data.py` 会**透明读取** `.csv.gz`，分析代码一行都不用改，`.csv` 和 `.csv.gz` 可以混存。
