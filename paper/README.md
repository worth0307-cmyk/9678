...
> **更正（2026-09-12）**：这一节原来写着「打平成本 6.38bp 对收 6.5bp，所以成不成立
> 完全取决于实际成交价」。那个数字是错的——它属于 **PCA 残差反转**（39/40 号脚本，
> 200 个币，年换手 441 次），不属于这里在跑的东西。

`44_paper_config.py` 把**正在跑的那个配置本身**（`vibt.paper.Params()`，一个参数
不改）回测了一遍：年化毛收益 +30.8%，年换手 65 次（单边），**打平成本 47.7bp/边**，
对回测假设的 6.5bp——成本要比假设差 **7 倍**才吃得掉它。

所以**成交价不是约束**。真正的约束是**选择偏差**：这六个币是在看过谁表现好之后
挑出来的，下面每个参数也都是在它们的历史上定的。

于是这个日志的第一件事是**预注册**：每条信号在结果揭晓之前就进了 git，这是日后
任何关于这套策略的说法还能被检验的唯一理由。量滑点是它的第二件事——它给估计值
划个边界，而不是决定成败。
...
bash ~/9678/scripts/vps_cron.sh --verify   # 昨夜那次跑对了没有（七项体检）
bash ~/9678/scripts/vps_cron.sh --status   # 看装了什么、日志到哪了
bash ~/9678/scripts/vps_cron.sh --remove   # 卸载
```

`--verify` 是每天/每周花十秒确认一次的东西。「跑了没有」不是一个是非题——
有七种各自独立的坏法，而其中几种**看起来像成功**：任务被别的 crontab 覆盖掉、
跑在错误的时刻、某个币抓取失败但脚本照常出表、ingest 声称成功却什么都没并进去、
写日志那一步被静默跳过。所以它一条一条查，全过才退出 0：

| # | 查什么 | 只看日志会漏掉的 |
|---|---|---|
| 1 | crontab 区块还在，**且整个 crontab 里只有一条** `vps_daily.sh` | 旧版把标记贴在行尾，那种行不在任何区块里、重装也删不掉，于是同一个任务装了两遍 |
| 2 | 最后一次运行是今天 | 漏跑几天，而日志里旧内容看着挺正常 |
| 3 | 跑在 `00:0x` UTC | 时区退回本机，每天都晚若干小时 |
| 4 | 这一次没报错 | —— |
| 5 | **`data/` 真的推进到最新一根已收盘日线** | 抓取/ingest 都「成功」却没并进去 |
| 6 | 信号写了或明确说了不是再平衡日，**且每个信号日只有一本账** | 两个实例并发跑，各写一份，行数翻倍 |
| 7 | 下一次的时刻 | —— |

第 4 项只看**最后一次**运行的那一段。日志是追加的，拿整个文件去 `grep 失败`
会把三个星期前早就修好的那次也算进来，于是每天都报错——**一个永远在响的检查
等于没有检查**。

第 5、6 两项里跑 python 的那段，「查出问题」用退出码 3，别的非零码一律报
「没结论」。否则 pandas 没装、CSV 读不动，都会被当成「数据落后」「有重复的账」
报出来——**一个分不清「查出问题」和「没查成」的检查，两种情况下都在骗人。**

### 单实例

`vps_daily.sh` 用 `flock` 保证同一时刻只有一个实例。两个一起跑是**看不出来**的那种
坏：它们并发重写同一批 `data/*.csv.gz`，而且都会在还空着的 `signals.csv` 上判定
「今天该再平衡」，于是同一天写进两本账，再平衡的节奏跟着算乱。拿不到锁的那个
安静退出（日志里留一行 `--- ... 跳过：另一个实例正在跑`，`--verify` 会提示）。
